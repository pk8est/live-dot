    ...
    render() {
        ...
        if (!disableMaximize) {
            maximizeButton = isMaximized ?
                <i className="iconfont maximize pull-right" onClick={ event.emit.bind(null, EVENT.MAXIMIZE_WINDOW) }>&#xe609;</i> :
                <i className="iconfont maximize pull-right" onClick={ event.emit.bind(null, EVENT.MAXIMIZE_WINDOW) }>&#xe606;</i>
        }
        return (<div className="titlebar">
                    <div className="titlebar-title">
                        ...
                        <span className="window-controls">
                            { closeAsHide ?
                                <i className="iconfont close pull-right" onClick={ event.emit.bind(null, EVENT.HIDE_WINDOW) }>&#xe602;</i> :
                                <i className="iconfont close pull-right" onClick={ event.emit.bind(null, EVENT.CLOSE_WINDOW) }>&#xe602;</i> }
                            { maximizeButton }
                            <i className="iconfont minimize pull-right" onClick={ event.emit.bind(null, EVENT.MINIMIZE_WINDOW) }>&#xe607;</i>
                        </span>
                    </div>
                </div>);
    }
